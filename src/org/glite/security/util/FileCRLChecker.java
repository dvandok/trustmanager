...
public class FileCRLChecker extends RevocationChecker {
	...
	/* TODO: remove this, do not store ca cert as it's not updated, make new method checkCRL with 
	 * cacert as input. */
	 ...
	 * 
	 * @param caCert
	 * @param caBaseFilename
	 *            the base CA filename that is used to load the crl.
	 * @param caNumber
	 *            the number of ca with the same DN.
	 * @param props
	...
	public FileCRLChecker(X509Certificate caCert, String caBaseFilename, int caNumber, CaseInsensitiveProperties props) {
		...
		if (crlReqText.startsWith("t") || crlReqText.startsWith("y")) {
			m_crlRequired = true;
		}
	 ...
	 *             if signature if wrong or the CRL has unsupported or invalid critical extensions.
	 * @throws IOException
	 *             in case CRL extension parsing fails.
	 */
	 ...
	 * 
	 * @param absolutePath
	 *            The filename of the CRL to load.
	 * @throws CertificateException
	 *             thrown in case there is problems with the certificate handling.
	 * @throws IOException
	...
	public void check(X509Certificate cert) throws IOException, CertificateException, CertificateRevokedException {
		...
		X509CRLEntry entry = m_crl.getRevokedCertificate(cert);
	 ...
	 * 
	 * @param oid
	 *            The oid of the extension to check.
	 * @throws CertificateException
	 *             thrown in case there is problems with the certificate handling.
	 * @throws IOException
	 *             thrown in case the extension parsing fails.
